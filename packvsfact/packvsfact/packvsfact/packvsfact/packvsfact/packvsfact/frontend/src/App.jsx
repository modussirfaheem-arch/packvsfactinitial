...
export default function App() {
  ...
  return (
    <Router>
      <div className="min-h-screen bg-[#faf7f2] text-stone-900 font-sans selection:bg-emerald-200 selection:text-emerald-900">
        ...
        {/* Auth Modal - Creamy Warm Vanilla & Emerald Theme */}
        {showAuthModal && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm animate-fadeIn">
            <div className="w-full max-w-md p-6 rounded-2xl bg-white border border-stone-200 shadow-xl relative">
              <button
              ...
              <div className="mb-4 p-3 rounded-xl bg-amber-50/70 border border-amber-200 text-xs">
                <span className="text-[11px] font-bold text-amber-900 block mb-1.5">Quick Demo Fill Credentials:</span>
                <div className="flex items-center space-x-2">
                  <button
                    type="button"
                    onClick={() => fillDemoAccount('USER')}
                    className="px-2.5 py-1 rounded bg-white hover:bg-amber-100 text-stone-800 border border-amber-300 font-semibold text-[11px]"
                  >
                    Consumer User
                  </button>
                  <button
              ...
              <form onSubmit={handleAuthSubmit} className="space-y-3.5">
                {!isLoginView && (
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-stone-50 border border-stone-300 rounded-xl px-3 py-2 text-xs text-stone-900 outline-none focus:border-emerald-600 focus:bg-white"
                    />
                  </div>
                )}
                ...
                {!isLoginView && (
                  <div>
                    <label className="text-[11px] font-semibold text-stone-600 block mb-1">Account Role</label>
                    <select
                      value={authRole}
                ...
                <button
                  type="submit"
                  className="w-full py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs shadow-xs transition"
                >
              ...
              <div className="mt-4 pt-3 border-t border-stone-100 text-center">
                <button
                  ...
                  className="text-xs text-stone-500 hover:text-emerald-700 font-semibold"
                >
