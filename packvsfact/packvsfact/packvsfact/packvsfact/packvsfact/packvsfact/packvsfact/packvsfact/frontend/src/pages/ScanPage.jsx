...
export default function ScanPage() {
  ...
  const [ingredientsText, setIngredientsText] = useState('Refined wheat flour, Palm oil, Sugar, Salt, INS 330, INS 621.');
      ...
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-xs sm:text-sm text-slate-400 mt-1">
            Our local OCR engine extracts calories, sugar, sodium, protein, fibre, and industrial additives. You can review and edit all extracted values before running final analysis.
          </p>
        </div>
        ...
        <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800 mb-8">
          <div className="flex flex-col md:flex-row items-center gap-6">
            <div className="w-full md:w-1/2 flex flex-col items-center justify-center p-8 border-2 border-dashed border-slate-700 hover:border-emerald-500/60 rounded-xl bg-slate-950/60 transition group cursor-pointer relative">
              <input
                type="file"
                accept="image/*"
              ...
              {previewUrl ? (
                ...
                <>
                  <Upload className="w-10 h-10 text-emerald-400 mb-3 group-hover:scale-110 transition-transform" />
                  <span className="text-xs font-bold text-slate-200">Click or Drag Nutrition / Ingredient Label Image</span>
                  <span className="text-[10px] text-slate-500 mt-1">Supports PNG, JPG, JPEG, WEBP (Max 10MB)</span>
                </>
              )}
            </div>

            <div className="w-full md:w-1/2 flex flex-col justify-center">
              <h3 className="text-sm font-bold text-slate-200 mb-2">OCR Processing Pipeline</h3>
        ...
        <div className="p-6 rounded-2xl bg-slate-900/80 border border-slate-800">
          <div className="flex items-center space-x-2 mb-4 border-b border-slate-800 pb-3">
            ...
            <h3 className="text-sm font-bold text-slate-100">User Confirmation & Value Verification</h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4 mb-6">
            <div>
              <label className="text-[11px] font-semibold text-slate-400 block mb-1">Product Name</label>
              <input
                type="text"
                value={productName}
            ...
            <div>
              <label className="text-[11px] font-semibold text-slate-400 block mb-1">Energy / Calories (kcal)</label>
              <input
                type="number"
                value={calories}
            ...
            <div>
              <label className="text-[11px] font-semibold text-slate-400 block mb-1">Total Sugar (g)</label>
              <input
                type="number"
                value={sugarG}
            ...
            <div>
              <label className="text-[11px] font-semibold text-slate-400 block mb-1">Dietary Fibre (g)</label>
              <input
                type="number"
                value={fibreG}
            ...
            <div>
              <label className="text-[11px] font-semibold text-slate-400 block mb-1">Sodium (mg)</label>
              <input
                type="number"
                value={sodiumMg}
          ...
          <div className="mb-6">
            <label className="text-[11px] font-semibold text-slate-400 block mb-1">Ingredients Statement</label>
            <textarea
              rows={3}
              value={ingredientsText}
              onChange={(e) => setIngredientsText(e.target.value)}
              className="w-full bg-slate-950 border border-slate-800 rounded-lg p-3 text-xs text-slate-200 outline-none focus:border-emerald-500"
            />
          </div>

          <button
            onClick={handleSubmitConfirmedData}
            className="w-full py-3 rounded-xl bg-gradient-to-r from-emerald-500 to-lime-500 hover:from-emerald-400 hover:to-lime-400 text-slate-950 font-bold text-sm shadow-md shadow-emerald-500/20 transition flex items-center justify-center space-x-2"
          >
